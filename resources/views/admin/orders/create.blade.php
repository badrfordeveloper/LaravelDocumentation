    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-start mb-0">{{ __('New') }} {{__('of')}} {{ __('order') }}</h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ url(BASE_ADMIN_URL) }}">{{ __('Dashboard') }}</a></li>
                            <li class="breadcrumb-item active">{{ __('New') }} {{__('of')}} {{ __('order') }}</li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card m-0">
                    ...
                    <div class="card-body">
                        <form class="form form-validation" method="POST"  action="{{ route(BASE_ADMIN_PATH.'.orders.store')}}" enctype="multipart/form-data"  >
                            @csrf
        ...
        <div class="content-wrapper p-0">
            <div class="content-body">
                <div class="bs-stepper checkout-tab-steps">
                    ...
                    <form id="checkout-form"  class="form form-validation" method="POST" action="{{ route(BASE_ADMIN_PATH.'.orders.store')}}" enctype="multipart/form-data">
                        @csrf
    ...
    @section('script')
        ...
        <script>
            $(document).on('change','select[name="customer_id"]', function (event) {
                ...
                var _url = $(this).attr('data-action') + "/" + _customer;
                $.ajax({
                    type: "GET",
                    url: _url,
                    success: function (response) {
                        if(response.success){
                            var _inputsDeliveredCustomer =_this.closest('.section-delivered-customer').find('input');
                            var _selectDeliveredCustomer =_this.closest('.section-delivered-customer').find('select').not(_this);
                            $.each(response.deliveredCustomer, function (indexitemCustomer , itemCustomer) {
                                // Chercher tous les input dans la section de delivered customer
                                $.each(_inputsDeliveredCustomer, function (indexInput, itemInput) {

                                    if(itemInput.name == indexitemCustomer){
                                        $(itemInput).val(itemCustomer);
                                    }
                                });
                                // Chercher tous les select dans la section de delivered customer
                                $.each(_selectDeliveredCustomer, function (indexSelect, itemSelect) {
                                    console.log(itemSelect.name + ' -- ' + indexitemCustomer)
                                    if(itemSelect.name == indexitemCustomer){
                                        // Chercher tous les options de select en cours
                                        $(itemSelect).find('option').each(function (indexOption, itemOption) {
                                            console.log(itemOption.value + " +++ " + itemCustomer);
                                            if(itemOption.value == itemCustomer){
                                                $(itemOption).prop('selected','selected').change();
                                            }
                                        });
                                    }
                                });
